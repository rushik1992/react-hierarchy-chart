...
export const HierarchyNode = ({
  ...
  return (
    <div
      className={`react-hierarchy-node-container react-hierarchy-node-container-${direction}`}
    >
      <div className={`react-hierarchy-node react-hierarchy-node-${direction}`}>
        {hasParent && (
          <div className={`node-line node-line-${direction}`}></div>
        )}
      ...
      {node.childs?.length && (
        <div
          className={`react-hierarchy-children react-hierarchy-children-${direction}`}
        >
          {node.childs.map((child, index) => {
            return (
              <div
                className={`children-container children-container-${direction}`}
                key={`HierarchyNode-${index}`}
              >
